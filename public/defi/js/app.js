  ...
  'AURA/USD': {
    ...
    generate: () => {
      ...
      return Array.from({length: 120}, (_, i) => {
        const trend = Math.sin(i / 18) * 0.008;
        const noise = (Math.random() - 0.5) * vol;
        const spike = Math.random() < 0.04 ? (Math.random() - 0.5) * 0.015 : 0;
        return base + trend + noise + spike + 0.0003 * i;
      });
    }
  },
  'ALEM/USD': {
    ...
    generate: () => {
      ...
      const vol = 0.35;
      return Array.from({length: 120}, (_, i) => {
        const trend = Math.cos(i / 24) * 0.5;
        const noise = (Math.random() - 0.5) * vol;
        const spike = Math.random() < 0.03 ? (Math.random() - 0.5) * 0.8 : 0;
        return base + trend + noise + spike - 0.002 * i;
      });
    }
  },
  'ETH/USD': {
    ...
    generate: () => {
      ...
      const vol = 45;
      return Array.from({length: 120}, (_, i) => {
        const trend = Math.sin(i / 8) * 30;
        const noise = (Math.random() - 0.5) * vol;
        const spike = Math.random() < 0.02 ? (Math.random() - 0.5) * 70 : 0;
        return base + trend + noise + spike + 1.2 * i;
      });
    }
  }
};
...
function drawTradingChart(pairKey) {
  ...
  lineGrad.addColorStop(1, pair.color + 'cc');
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.fillText(label, padL + plotW - textW - 4, y + 4);
  }
  ...
  document.getElementById('tradingPrice').textContent =
    lastPrice < 1 ? '$' + lastPrice.toFixed(4) : lastPrice < 100 ? '$' + lastPrice.toFixed(2) : '$' + lastPrice.toFixed(2);
  document.getElementById('tradingPrice').className = 'trading-price' + (isUp ? ' up' : ' down');
  document.getElementById('tradingChange').textContent =
