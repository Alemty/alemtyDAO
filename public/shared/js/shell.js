...
// ✅ NUEVO: export para reutilizar credenciales fuera del drawer
export async function mountCreds(root = document) {
  ...
  // ✅ Shuffle local (solo se usa dentro de mountCreds)
  ...
  if (poaps && Array.isArray(poaps) && poaps.length) {
    ...
    poapEl.innerHTML = `<div class="poap-row" id="poapRow">
      <div class="poap-marquee">
        <div class="poap-track">${filled}</div>
        <div class="poap-track" aria-hidden="true">${filled}</div>
      </div>
    </div>`;

    const poapRow = poapEl.querySelector("#poapRow");
    if (poapRow) {
      poapRow.style.setProperty("--poap-duration", "36s");
      pauseOnHold(poapRow);
    }
  } else {
  ...
  tabsEl.innerHTML = cats.map((cat,i) =>
    `<button class="cert-tab ${i===0?"active":""}" type="button" data-cat="${esc(cat)}">
      ${esc(cat)} <span class="n">${readyByCat[cat].length}</span>
    </button>`
  ).join("");
  ...
  stripEl.innerHTML = cats.map((cat,i) => {
    ...
    const base = ready.map((c, realIdx) => {
      ...
      return { html:
        `<a class="cert-badge" href="${esc(href)}" target="_blank" rel="noopener noreferrer"${dis}
           data-cat="${esc(cat)}" data-idx="${realIdx}">
           ${img}<span class="label">${title}</span>
         </a>`
      };
    });
    ...
    return `<div class="cert-row ${i===0?"active":""}" data-cat="${esc(cat)}">
      <div class="cert-marquee">
        <div class="cert-track">${tiles}</div>
        <div class="cert-track" aria-hidden="true">${tiles}</div>
      </div>
    </div>`;
  }).join("");
  ...
  detailEl.innerHTML = `<div class="t">${esc(cats[0])}</div>
    <div class="m small muted">Selecciona una medalla para ver detalles.</div>`;
  ...
  tabsEl.querySelectorAll(".cert-tab").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      stripEl.querySelectorAll(".cert-row").forEach(r =>
        r.classList.toggle("active", r.getAttribute("data-cat") === cat)
      );
      detailEl.innerHTML = `<div class="t">${esc(cat)}</div>
        <div class="m small muted">Selecciona una medalla para ver detalles.</div>`;
    });
  });
  ...
  stripEl.querySelectorAll(".cert-badge").forEach(a => {
    a.addEventListener("click", (e) => {
      ...
      detailEl.innerHTML = `<div class="t">${title}</div>
        <div class="m small muted">${issuer}${year} · ${category}</div>
        <div class="actions">
          <a class="cert-cta" href="${esc(url)}" target="_blank" rel="noopener noreferrer">Ver certificado</a>
        </div>`;
    });
  });
