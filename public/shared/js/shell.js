...
/* =========================================================
   GLOBAL PAGE SCROLL INDICATOR (VERTICAL) — Shell-level
   Visible en todas las subapps (ID/DAO/DEFI)
========================================================= */
let __scrollRAF = 0;
...
function ensureScrollIndicator() {
  let host = document.getElementById('shellScrollIndicator');
  if (host) return host;

  host = document.createElement('div');
  host.id = 'shellScrollIndicator';
  host.setAttribute('aria-hidden', 'true');

  // respeta topbar/bottom-nav
  const topOff = 84;   // ~topbar + aire
  const botOff = 96;   // ~bottom-nav + aire
...
function updateScrollIndicator() {
  const host = ensureScrollIndicator();
  const thumb = host.querySelector('.thumb');
  if (!thumb) return;

  // Si hay cualquier modal abierto (DAO modal / profile / notif / etc), ocultamos el indicador
...
function scheduleScrollIndicator() {
...
window.addEventListener('scroll', scheduleScrollIndicator, { passive: true });
window.addEventListener('resize', scheduleScrollIndicator, { passive: true });
