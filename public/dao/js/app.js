...
// =========================
// Page scroll indicator (VERTICAL) — JS-only (no depende del OS)
// =========================
let __pageScrollRAF = 0;

function getDaoTopbarOffsetPx() {
  // Usa el token CSS si existe: --dao-topbar-h (72px) + aire
  try {
    const v = getComputedStyle(document.documentElement).getPropertyValue('--dao-topbar-h').trim();
    const n = parseFloat(v);
    if (Number.isFinite(n)) return n + 16;
  } catch {}
  return 88; // fallback razonable (72 + 16)
}

function getDaoBottomNavOffsetPx() {
  // El shell trae bottom nav fija; dejamos un aire
  return 96; // ~78px + aire
}
...
function getScrollState() {
  ...
  const top = de.scrollTop || b.scrollTop || 0;
  const sh = de.scrollHeight || b.scrollHeight || 0;
  const vh = window.innerHeight || de.clientHeight || 0;
  return { top, sh, vh };
}

function ensurePageScrollIndicator() {
  let host = document.getElementById('pageScrollIndicator');
  if (host) return host;

  host = document.createElement('div');
  host.id = 'pageScrollIndicator';
  host.setAttribute('aria-hidden', 'true');

  const topOff = getDaoTopbarOffsetPx();
  const botOff = getDaoBottomNavOffsetPx();
...
function updatePageScrollIndicator() {
  const host = ensurePageScrollIndicator();
  const thumb = host.querySelector('.thumb');
  if (!thumb) return;

  // Si hay modal abierto, la página usualmente se queda “de fondo”.
  // Puedes ocultarlo o dejarlo: aquí lo ocultamos para no estorbar.
  const daoModal = document.getElementById('daoModal');
  if (daoModal && daoModal.classList.contains('open')) {
    host.style.display = 'none';
    return;
  }
  host.style.display = '';

  const { top, sh, vh } = getScrollState();
  if (sh <= vh + 2) {
    host.style.display = 'none';
    return;
  }
  ...
  const trackH = host.getBoundingClientRect().height;
  const minThumb = 30;

  const thumbH = Math.max(minThumb, Math.round(trackH * (vh / sh)));
  const maxY = Math.max(0, trackH - thumbH);
  const y = Math.round((top / maxScroll) * maxY);

  thumb.style.height = `${thumbH}px`;
  thumb.style.transform = `translateY(${y}px)`;
}

function schedulePageIndicator() {
  if (__pageScrollRAF) return;
  __pageScrollRAF = requestAnimationFrame(() => {
    __pageScrollRAF = 0;
    updatePageScrollIndicator();
  });
}

// Listeners (no conflict, passive)
window.addEventListener('scroll', schedulePageIndicator, { passive: true });
window.addEventListener('resize', schedulePageIndicator, { passive: true });

// primer paint
requestAnimationFrame(updatePageScrollIndicator);
